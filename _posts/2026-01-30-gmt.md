...
![An overview of GMT](../assets/img/blogs/GMT.png)
...
> a. The trained **Teacher Policy** controls the robot in the simulation.
> b. Detailed data is recorded for every frame:
...
> a. Feed the recorded data into the Student Policy.
> b. **Forward Pass:** The Student predicts action $a_{t}$ based on history.
> c. **Loss Calculation:** Compare the difference between $a_t$ and $\hat{a}_t$.
> d. **Backpropagation:** Update the Student network's weights.
> 3. **DAgger (Dataset Aggregation)**
> a. To prevent **Distribution Shift** (where the Student makes a small mistake, enters a state the Teacher never visited, and fails), DAgger is often used.
> b. **Mixed Control:** During data collection, the Student is allowed to control the robot. When it is about to fall or deviate, the Teacher takes over to correct it.
> c. This collects "correction" data, teaching the Student: "if you deviate, here is how to recover."
> 4. **Sim-to-Real Deployment:**
> a. Once training is complete, only the **Student Policy** weights are exported.
> b. This network is lightweight, fast, and does not rely on privileged info, making it ready for deployment on real robot hardware.
